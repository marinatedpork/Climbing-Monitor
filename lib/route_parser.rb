...
class RouteParser
  def self.scrape_some_shit(link_file_to_scrape_from)
    ...
    links.each do |link|
      ...
      CSV.open("routes.csv", "a+") { |csv| csv << [scraper.route_name, scraper.route_type, scraper.route_name, scraper.route_rating_as_int, scraper.height_or_nil, scraper.route_name, scraper.pitches, new_link.strip] }
    end
  end
end
